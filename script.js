...
function sendMail(event) {
  ...
  emailjs
    ...
    .catch((error) => {
      // Show error notification with detailed logging
      notificationMessage.textContent = "Failed to send message. Please try again.";
      notification.classList.add("show");
      setTimeout(() => notification.classList.remove("show"), 3000);
      console.error("EmailJS Error: ", error); // Log full error for debugging
    });
}
...
// Close notification when the close button is clicked (moved outside sendMail)
